...
def test_signup(session: Session, anonymous_client: TestClient):
    ...
    response, response_data = _make_request(anonymous_client, "post", "/signup", data)
...
def test_signin(anonymous_client: TestClient, user_data: UserData):
    ...
    response, response_data = _make_request(anonymous_client, "post", "/signin", data)
...
def test_list_users(client: TestClient, users: list[User]):
    response, response_data = _make_request(client, "get", "/users/")
...
def test_get_profile(client: TestClient, user: User):
    response, response_data = _make_request(client, "get", "/profile/")
...
def test_update_profile(client: TestClient, session: Session, user: User):
    ...
    response, response_data = _make_request(client, "patch", "/profile/", data)
...
def test_delete_profile(client: TestClient, session: Session, user: User):
    response, response_data = _make_request(client, "delete", "/profile/")
    assert response.status_code == status.HTTP_200_OK
...
def test_change_password(client: TestClient, session: Session, user_data: UserData):
    ...
    response, response_data = _make_request(client, "patch", "/profile/password", data)
    assert response.status_code == status.HTTP_200_OK
